...
void add_casio_task_2_list(struct casio_rq* rq, struct task_struct* p){
	...
	if (rq && p){
		...
		if (new){
			...
			//logs
			sniprintf(msg,CASIO_MSG_SIZE,"add_casio_task_2_list: %d:d%:%llu", new->task->casio_id, new->task->pid, new->absolute_deadline);
			register_casio_event(sched_clock(),msg,CASIO_MSG);
		} else {
...
static void enqueue_task_casio(struct rq* rq, struct task_struct* p, int wakeup)
{
	...
	if (p){
		...
		if (t){
			...
			//logs
			snprintf(msg, CASIO_MSG_SIZE,"(%d:%d:$11u)",p->casio_id, p->pid, t->absolute_deadline);
